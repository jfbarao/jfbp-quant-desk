# =========================================================
# JFBP QUANT DESK — SIGNAL WATCHER v3.0 ANALYTICS
# Background BUY-signal alert engine — Scanner BUY logic + analytics
# ---------------------------------------------------------
# Purpose:
...
#   ✅ Logs every scan, alert, and signal row to CSV
#   ✅ Writes operational analytics for Automation Control Center
#
# What it does NOT do:
#   ❌ No OMS routing
#   ❌ No IBKR orders
...
WATCHER_VERSION = "v3.0-analytics"
WATCHER_SOURCE = "jfbp_signal_watcher_v3_analytics"
...
def run_scan(config: WatcherConfig) -> Dict[str, Any]:
    ...
    started = utc_now_iso()
    scan_id = started.replace(":", "").replace("+", "_")
    ...
    # v3 analytics: log every signal row so the Control Center can show BUY/WATCH/SELL counts.
    recommendation_counts: Dict[str, int] = {}
    signal_counts: Dict[str, int] = {}
    sector_counts: Dict[str, int] = {}
    top_buy_symbols: List[str] = []

    for row in rows:
        rec = row.trade_recommendation.upper().strip() or "UNKNOWN"
        sig = row.signal.upper().strip() or "UNKNOWN"
        ...
        if rec in ("BUY", "STRONG BUY"):
            top_buy_symbols.append(row.symbol)
    ...
    stats.update(
        {
            ...
            "last_scan_status": "OK",
            "last_scan_recommendation_counts": recommendation_counts,
            "last_scan_signal_counts": signal_counts,
            "last_scan_sector_counts": sector_counts,
            "last_scan_buy_symbols": top_buy_symbols[:20],
        }
    )
    ...
    append_csv(
        SCAN_LOG_FILE,
        {
            ...
            "strong_buy_count": recommendation_counts.get("STRONG BUY", 0),
            "watch_count": recommendation_counts.get("WATCH", 0),
    ...
    print(
        f"[{local_now_text()}] Scan finished: alerts={alerts_sent}, duplicates={duplicates_blocked}, "
        f"errors={errors}, buys={recommendation_counts.get('BUY', 0)}, "
        f"strong_buys={recommendation_counts.get('STRONG BUY', 0)}, watches={recommendation_counts.get('WATCH', 0)}"
    )

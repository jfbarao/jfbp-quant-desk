...
class PortfolioEngine:
    ...
    def apply_fill(self, fill: Any) -> Dict[str, Any]:
        ...
        if collision and not replay_mode:
    ...
    def _broker_rows(self, data: Any) -> List[Dict[str, Any]]:
        """
        Convert broker/dataframe/object/dict inputs into list[dict].
        """
    ...
    def normalize_broker_positions(
        ...
        """
        Convert broker snapshot rows into canonical signed position rows.
        ...
        for row in self._broker_rows(broker_positions):
            ...
            if contract is not None:
                ...
                contract_local_symbol = getattr(contract, "localSymbol", "") or ""
                contract_sec_type = getattr(contract, "secType", "") or ""
            ...
            avg_price = self._float(
                ...
                else row.get("averageCost")
                if row.get("averageCost") is not None
                else row.get("price")
    ...
    def detect_broker_drift(
        ...
        """
        Compare broker position truth against portfolio runtime truth.
        This is read-only.
        """
        ...
        qty_mismatches = []
        avg_price_mismatches = []

        for symbol in sorted(runtime_symbols - broker_symbols):
            runtime_qty = self._float(runtime.get(symbol, {}).get("signed_qty"))
        ...
        for symbol in sorted(broker_symbols - runtime_symbols):
            broker_qty = self._float(broker.get(symbol, {}).get("signed_qty"))

            if abs(broker_qty) > tolerance:
                unexpected_in_broker.append({
                    "symbol": symbol,
        ...
        for symbol in sorted(broker_symbols & runtime_symbols):
            ...
            broker_avg = self._float(broker.get(symbol, {}).get("avg_price"))
            runtime_avg = self._float(runtime.get(symbol, {}).get("avg_price"))

            if (
                broker_avg > 0
                and runtime_avg > 0
                and abs(broker_avg - runtime_avg) > tolerance
            ):
                avg_price_mismatches.append({
                    "symbol": symbol,
                    "broker_avg_price": broker_avg,
                    "runtime_avg_price": runtime_avg,
                    "delta": broker_avg - runtime_avg,
    ...
    def import_broker_snapshot(
        ...
        """
        Import broker positions as runtime position state.

        dry_run=True returns what would change.
        dry_run=False mutates positions/lots/last_prices.

        This does NOT create fills and does NOT execute trades.
        """
        ...
        if not broker:
            report = {
                "timestamp": self._now(),
                "status": "ABORTED",
                "reason": "Empty broker snapshot. Import refused.",
                "broker_positions": 0,
                "runtime_positions_before": len(before),
                "runtime_positions_after": len(before),
                "actions": [],
                "actions_count": 0,
                "before": before,
                "after": before,
            ...
            self.last_broker_snapshot_report = report
            return report

        actions = []

        for symbol, row in sorted(broker.items()):
            broker_qty = self._float(row.get("signed_qty"))
            broker_avg = self._float(row.get("avg_price"))
            ...
            realized = old_realized if preserve_realized_pnl else 0.0

            if (
                abs(old_qty - broker_qty) > self.EPSILON
                or abs(old_avg - broker_avg) > self.EPSILON
            ):
                actions.append({
                    "action": "IMPORT_BROKER_POSITION",
                    "symbol": symbol,
                    "old_qty": old_qty,
                    "new_qty": broker_qty,
                    "old_avg_price": old_avg,
                    "new_avg_price": broker_avg,
            ...
            if not dry_run:
                ...
                self.lots[symbol] = [
                    Lot(
                        ...
                        qty=broker_qty,
                        price=broker_avg,
                        fill_id="BROKER_SNAPSHOT_IMPORT",
                        timestamp=self._now(),
                    )
                ]

                if broker_avg > 0:
                    self.last_prices[symbol] = broker_avg

        for symbol, pos in list(self.positions.items()):
            if symbol in broker:
                continue

            old_qty = self._float(pos.quantity)
            old_realized = self._float(pos.realized_pnl)

            if abs(old_qty) > self.EPSILON:
                actions.append({
                    "action": "RUNTIME_POSITION_NOT_IN_BROKER",
                    "symbol": symbol,
                    "old_qty": old_qty,
                    "broker_qty": 0.0,
        ...
        report = {
            ...
            "broker_positions": len(broker),
            "runtime_positions_before": len(before),
            "runtime_positions_after": len(after),
            "actions": actions,
    ...
    def flatten_runtime_orphans(
        ...
        """
        Remove runtime positions that do not exist at broker.
        This is a runtime repair only. It does NOT send broker orders.
        """
        ...
        for symbol, pos in list(self.positions.items()):
            ...
            realized = self._float(pos.realized_pnl)
            ...
            actions.append({
                ...
                "old_qty": qty,
                "new_qty": 0.0,
                "preserved_realized_pnl": realized if preserve_realized_pnl else 0.0,
            })
            ...
            if not dry_run:
                self._set_position(
                    ...
                    quantity=0.0,
                    avg_price=0.0,
                    realized_pnl=realized if preserve_realized_pnl else 0.0,
                )
    ...
    def rebuild_from_broker_snapshot(
        ...
        """
        Rebuild runtime position state from broker positions.

        This does NOT rebuild audit history.
        This does NOT execute broker trades.
        This optionally preserves realized P&L by symbol.

        Critical safety:
        Empty broker snapshots are refused.
        An empty snapshot can mean IBKR was disconnected, stale, unavailable,
        or not pulled yet. It must never wipe runtime positions.
        """
        ...
        for symbol, row in sorted(broker.items()):
            ...
            old_avg = self._float(old.get("avg_price"))
            new_avg = self._float(row.get("avg_price"))

            if (
                abs(old_qty - new_qty) > self.EPSILON
                or abs(old_avg - new_avg) > self.EPSILON
            ):
                actions.append({
        ...
        for symbol in sorted(set(before.keys()) - set(broker.keys())):
            actions.append({
                ...
                "symbol": symbol,
                "old_qty": self._float(before.get(symbol, {}).get("signed_qty")),
                "new_qty": 0.0,
            })
    ...
    def broker_vs_ledger_reconcile(
        self,
        broker_positions: Any,
        audit_fills: Optional[List[Dict[str, Any]]] = None,
    ) -> Dict[str, Any]:
        """
        Compare broker position truth against current portfolio ledger
        and optional audit fills.
        """

        broker_drift = self.detect_broker_drift(broker_positions)

        ledger_rows = [
            self._normalize_fill(row)
            for row in self.ledger
            if self._is_true_fill(self._normalize_fill(row))
        ]

        audit_rows = [
            self._normalize_fill(row)
            for row in (audit_fills or [])
            if self._is_true_fill(self._normalize_fill(row))
        ]
        ...
        for key, row in audit_map.items():
        ...
        for key, row in ledger_map.items():
            if audit_rows and key not in audit_map:
                missing_in_audit.append({
                    "identity_key": key,
                    "symbol": row.get("symbol"),
                    "action": row.get("action"),
                    "qty": row.get("qty"),
        ...
        if (
            broker_drift.get("status") != "MATCH"
            or missing_in_ledger
            or missing_in_audit
        ):
            status = "DRIFT"
    ...
    def broker_execution_repair(
        ...
        """
        Apply missing broker execution fills into portfolio ledger.

        Requires broker fills with executable fields:
        symbol, action/side, qty, price, execution_id/exec_id preferred.

        dry_run=True reports missing fills only.
        dry_run=False calls apply_fill() in replay/broker repair mode.
        """
        ...
        for key, row in sorted(broker_map.items()):
            if key in ledger_map:
                skipped.append({
                    "identity_key": key,
                    "symbol": row.get("symbol"),
                    "reason": "Already in portfolio ledger",
                })
                continue

            missing_in_ledger.append(row)
    ...
    def repair_from_broker_truth(
        ...
        """
        Top-level broker authority repair coordinator.
        ...
        dry_run=True is strongly recommended before applying.
        """
        ...
        if not broker and mode in ("BROKER_POSITIONS_ONLY", "HYBRID_CONSENSUS"):
            before = self.snapshot()
            ...
            report = {
                ...
                "drift_report": {
                    "timestamp": self._now(),
                    "status": "ABORTED",
                    "reason": "Empty broker snapshot. Drift check refused for repair coordinator.",
                    "broker_positions": 0,
                    "runtime_positions": len(before),
                    "broker_truth": {},
                    "runtime_truth": before,
                    "truth_source": self.TRUTH_SOURCE,
                },
                "execution_report": {},
                "position_report": {
                    "status": "ABORTED",
                    "reason": "Empty broker snapshot. Position rebuild refused.",
                    "before": before,
                    "after": before,
                },
                "truth_source": self.TRUTH_SOURCE,
        ...
        if position_report.get("status") == "ERROR":
            final_status = "ERROR"

        if position_report.get("status") == "ABORTED":
            final_status = "ABORTED"

        if (
            execution_report
            and execution_report.get("status") == "PARTIAL_REPAIR"
        ):
